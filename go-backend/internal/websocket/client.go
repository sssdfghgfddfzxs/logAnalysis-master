...
func (c *Client) handleMessage(message []byte) {
	...
	if !ok {
		log.Printf("Invalid message type from client")
		return
	}
	...
	case "ping":
		...
		if data, err := json.Marshal(pongMsg); err == nil {
			...
			case c.send <- data:
			default:
				// Channel is full, client will be disconnected
			}
		}

	case "subscribe":
		// Handle subscription requests (future enhancement)
		log.Printf("Client subscription request: %v", msg)

	case "unsubscribe":
		// Handle unsubscription requests (future enhancement)
		log.Printf("Client unsubscription request: %v", msg)
